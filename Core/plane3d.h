...
#ifndef __KK_PLANE_3D_
#define __KK_PLANE_3D_
...
class plane3d
{
	public:
		...
		plane3d(): Normal(0,1,0) { recalculateD(core::vector3d<T>(0,0,0)); };
		plane3d(const vector3d<T>& MPoint, const core::vector3d<T>& Normal) : Normal(Normal) { recalculateD(MPoint); };
		...
		plane3d(const core::vector3d<T>& point1, const core::vector3d<T>& point2, const core::vector3d<T>& point3) { setPlane(point1, point2, point3); };
		...
		void setPlane(const core::vector3d<T>& point, const core::vector3d<T>& nvector)
		{
			Normal = nvector;
			Normal.normalize();
			recalculateD(point);
		...
		void setPlane(const core::vector3d<T>& nvect, T d)
		{
			Normal = nvect;
			D = d;
		}

		void setPlane(const core::vector3d<T>& point1, const core::vector3d<T>& point2, const core::vector3d<T>& point3)
		{
		...
		bool getIntersectionWithLine(const core::vector3d<T>& linePoint, const core::vector3d<T>& lineVect,
					core::vector3d<T>& outIntersection) const
		{
		...
		f32 getKnownIntersectionWithLine(const core::vector3d<T>& linePoint1,
			const core::vector3d<T>& linePoint2) const
		{
			core::vector3d<T> vect = linePoint2 - linePoint1;
		...
		//! \param outIntersection: Place to store the intersection point, if there is one.
		//! \return Returns true if there was an intersection, false if there was not.
		bool getIntersectionWithLimitedLine(const core::vector3d<T>& linePoint1, 
					const core::vector3d<T>& linePoint2, core::vector3d<T>& outIntersection) const
		{
		...
		//is point infront of plane ,or in plane's back side,or on the plane
		EIntersectionRelation3D classifyPointRelation(const core::vector3d<T>& point) const
		{
		...
		void recalculateD(const core::vector3d<T>& MPoint)
		{
			D = - MPoint.dotProduct(Normal);
		}

		//! Returns a member point of the plane.
		core::vector3d<T> getMemberPoint() const
		{
			return Normal * -D;
		}
		...
		{
			core::vector3d<T> cross = other.Normal.crossProduct(Normal);
			return cross.getLength() > core::ROUNDING_ERROR_32;
		}

		//! Intersects this plane with another.
		//! \return Returns true if there is a intersection, false if not.
		bool getIntersectionWithPlane(const plane3d<T>& other, core::vector3d<T>& outLinePoint,
				core::vector3d<T>& outLineVect) const
		{
		...
		bool getIntersectionWithPlanes(const plane3d<T>& o1, 
				const plane3d<T>& o2, core::vector3d<T>& outPoint) const
		{
			core::vector3d<T> linePoint, lineVect;
		...
		bool isFrontFacing(const core::vector3d<T>& lookDirection) const
		{
		...
		//! works if the normal is Normalized.
		T getDistanceTo(const core::vector3d<T>& point) const
		{
		...
		core::vector3d<T> Normal;		// normal vector
